...
// namespace ManyMailer\libraries\DebugHelper;
if (!defined('BASEPATH')) {
...
class Debughelper
{
    ...
    {
        if (is_null($type)) {
            throw new \Exception('Missing (type) in message construct');
        }
        if (is_null($method)) {
            throw new \Exception('Missing (method) in message construct');
        }
        if (is_null($style)) {
     ...
     * @return type
     */
    public function strByType($search)
    {
        $searchResults = array();
    ...
    public function _generate_messages($struct)
    {
        $this->add_messages($struct);
    }

    public function _isClose($method)
    {
        return array_search($method, array_combine(array_keys(self::GRP_CLOSED), array_values(self::GRP_CLOSED)));
    }
           ...
           foreach ($sect as $msg) {                  // compile msg
               ...
               switch ($method) {
                    case ']':               // NOTE: fall through handles multiple syntaxes
                        $method = DebugHelper::GRP_CLOSED[$method]; // get console method
                        // no break
                    case 'groupEnd':
    ...
    public function _get_stack_info($valueObj)
    {
        /*
        * set color tone **/
        $dbg_messages = array();
        ...
        $detailLogLevel = ($valueObj['exit']) ? 'error' : 'info';
        $logLevel = ($valueObj['exit']) ? 'rainbow' : $defaultStyle;
        ...
        $bt = debug_backtrace();
        $bt_obj = $bt[self::BT_OFFSET + $unknown_error_offset];
        $prev_bt_obj = $bt[self::BT_OFFSET + $unknown_error_offset + 1];
        $caller = json_encode(array_slice($bt, self::BT_OFFSET + $unknown_error_offset), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_NUMERIC_CHECK);
        ...
        $caller_class = (isset($bt_obj['class'])) ? $bt_obj['class'] : lastClass($bt, self::BT_OFFSET + $unknown_error_offset + 1);
        ...
        $str_msg = array(
            'str' => '%cString Val: '.preg_replace('/[\x00-\x1F]/u', '', $value),
    ...
    public function genericErrorHandler($errno, $errstr, $errfile, $errline)
    {
        ...
        $this->c_log($msg."::$errline", $errstr, true);
