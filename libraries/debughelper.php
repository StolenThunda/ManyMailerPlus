...
class Debughelper
{
    ...
    private const BT_OFFSET = 2;
    private const BT_LINE_OFFSET = 1;
        ...
        foreach ($this->sections as $section) {        // loop all sections to get message order
           ...
           foreach ($sect as $msg) {                  // compile msg
               ...
               switch ($method) {
                    ...
                    case '[':               // NOTE: fall through handles multiple syntaxes
                    case '[[':               // NOTE: fall through handles multiple syntaxes
                        $method = debug_helper::GRP_OPEN[$method]; // get console method
                        // no break
                    case 'groupEnd':
    ...
    public function console_message($value, $title = null, $exit = false)
    {
        $str = '';
        $valueObj = array(
            'value' => $value,
    ...
    public function _consoleString($valueObj, $title = null, $exit = false)
    {
        /*
        * set color tone **/
        $dbg_messages = array();
    ...
    public function genericErrorHandler($errno, $errstr, $errfile, $errline)
    {
        ...
        ee()->dbg->console_message($msg."::$errline", $errstr, true);
