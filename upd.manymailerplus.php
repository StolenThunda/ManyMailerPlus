...
class Manymailerplus_upd {
	...
	function uninstall()
	{
		...
		//ee()->dbg->console_message(ee()->db->last_query(), ee()->db->affected_rows()." Rows Deleted", TRUE);

        ee()->load->dbforge();
		foreach (array('csv_object', 'mailKey') as $column){
			ee()->dbg->console_message("$column TEST", __METHOD__);
			if( ee()->db->field_exists($column, 'exp_email_cache')){
				...
				ee()->dbg->console_message("$column does not exist", __METHOD__);
			}
        }
		return TRUE;
	}
	...
	function addCsvColumn(){
		...
		foreach (array_keys($fields) as $column){
			...
			// ee()->dbg->console_message(ee()->db->last_query(), ee()->db->affected_rows()." Rows Affected");
			// ee()->dbg->console_message($column,"Has Column : ".(($hasColumns) ? 'TRUE': 'FALSE'));
			if ($hasColumns) break;
		} 
		
		if (!$hasColumns) {
			$result = ee()->dbforge->add_column('email_cache', $fields);
			// ee()->dbg->console_message($result->num_rows(), (string) $hasColumns);
		}
